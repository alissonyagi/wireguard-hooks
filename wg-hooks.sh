...
RESOLV_PATH="/etc/netns/${NETNS}"
RESOLV="${RESOLV_PATH}/resolv.conf"
...
if [ "${OP}" = "up" ]; then
	...
	if [[ ${#DNS[@]} -gt 0 ]]; then
		if [ ! -f "/etc/netns/${NETNS}/resolv.conf" ]; then
			mkdir -p "${RESOLV_PATH}" 2>/dev/null
			echo "# This file was automatically generated by wireguard-hooks" >> "${RESOLV}"
		fi

		for nameserver in "${DNS[@]}"; do
			NAMESERVER_EXISTS=$(grep -E "^(nameserver ${nameserver})$" "${RESOLV}")

			if [ -z "${NAMESERVER_EXISTS}" ]; then
...
if [ "${OP}" = "down" ]; then
	...
	RESOLV_EXISTS=$(head -n1 "${RESOLV}" 2>/dev/null | grep -E "(generated by wireguard-hooks)$")

	if [ -n "${RESOLV_EXISTS}" ]; then
		rm -f "${RESOLV}"
	fi
